...
router.post('/farmer-signup', async (req, res) => {
    const { name_farmer, aadharnumber, role, locality, state, address, pincode, phonenumber, email, password } = req.body;
...
router.post('/transporter-signin', async (req, res) => {
    try {
        ...
        if (userLogin.role != user_role) {
            console.log("Cannot login")
            return res.status(422).json({ error: "Cannot login" });
        } else {
...
router.get('/gettransportingrequest', farmerauthenticate, async (req, res) => {
    const userEmail = req.rootUser.email
    // console.log(userEmail)
    const data = await TransportingRequest.find({ email_farmer: userEmail })
    res.send(data)
})
router.get('/requests', farmerauthenticate, async (req, res) => {
    ...
    if (userRole === "Farmer") {
        ...
        const data = await TransportingRequest.find({ email_buyer: userEmail })
        res.send(data)
    }
    // console.log(userEmail)
...
router.post('/accept', async (req, res) => {
    ...
    try {
        ...
        console.log(update)
        if (update === null) {
            const update1 = await BuyingRequest.findOneAndUpdate({ _id: Request_Id }, { type: "Accepted", })
            console.log(update1)
            if (update1 === null) {
                const update2 = await TransportingRequest.findOneAndUpdate({ _id: Request_Id }, { type: "Accepted", })
                console.log(update2)
            }
        }
        res.status(200).json("Change Successful")
...
router.post('/decline', farmerauthenticate, async (req, res) => {
    ...
    try {
        ...
        if (update === null) {
            ...
            const update = await TransportingRequest.findOneAndUpdate({ _id: Request_Id }, { type: "Declined", })
            console.log(update)
        }
        res.status(200).json("Change Successful")
    } catch (error) {
